...
def standard_snipping_strategy():
    return Triplet1DSnippingStrategy(
        bin_size=50_000, half_window_size=100_000, snipping_value=SnippingValues.ICCF, position_slack=1_000_000
    )

@pytest.fixture
def center_snipping_strategy():
    return Triplet1DSnippingStrategy(
        bin_size=50_000, half_window_size=100_000, snipping_value=SnippingValues.ICCF, position_slack=0
    )

@pytest.fixture
def center_snipping_strategy_w_offset():
    return Triplet1DSnippingStrategy(
        bin_size=50_000, half_window_size=100_000, snipping_value=SnippingValues.ICCF, position_slack=0, relative_offset=50_000
    )
...
def standard_snipping_strategy_obs_exp(single_region):
    return Triplet1DSnippingStrategy(
        bin_size=50_000, half_window_size=100_000, snipping_value=SnippingValues.OBSEXP, position_slack=1_000_000,
        n_random_regions=100, genome="hg19"
    )
...
def test_entire_region_with_complete_pixels(
    complete_synthetic_pixels, single_region, expected_entire_single_region_complete, standard_snipping_strategy
):
    """Test whether snipping of an entire region produces correct results when pixels are supplied as dataframe."""
    # do the snipping
    result = standard_snipping_strategy.snip(
...
def test_entire_region_with_complete_pixels_from_file(
    complete_persisted_pixels, single_region, expected_entire_single_region_complete, standard_snipping_strategy
):
...
def test_two_regions_with_complete_pixels(
    complete_synthetic_pixels, two_regions, expected_two_entire_regions_complete, standard_snipping_strategy
):
...
def test_entire_region_with_incomplete_pixels(
    incomplete_synthetic_pixels, single_region, expected_entire_single_region_incomplete, standard_snipping_strategy
):
...
def test_two_regions_with_incomplete_pixels(
    incomplete_synthetic_pixels, two_regions, expected_two_entire_regions_incomplete, standard_snipping_strategy
):
...
def test_one_region_center_selection(
    complete_synthetic_pixels, single_region, expected_one_region_center_selection, center_snipping_strategy
):
    """Tests whether snipping of complete region with center selection produces correct results"""
    # do the snipping
    result = center_snipping_strategy.snip(
        complete_synthetic_pixels,
        single_region
    )
    # check result
...
def test_two_regions_center_selection(
    complete_synthetic_pixels, two_regions, expected_two_regions_center_selection, center_snipping_strategy
):
...
def test_relative_offset_works(
    complete_synthetic_pixels, single_region, expected_one_region_50k_offset_selection, center_snipping_strategy_w_offset
):
    # do the snipping
...
def test_one_region_center_selection_obs_exp(
    ...
    mocker.patch.object(Triplet1DSnippingStrategy, '_get_random_coordinates', return_value=single_region)
    # do the snipping
    ...
    np.testing.assert_array_almost_equal(
        result.values, np.ones(result.values.shape)
    )
