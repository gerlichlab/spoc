...
def test_offset_pixels_left_string_arg(genomic_data_fixture, request):
    """Tests offset calculation succeeds for pixels with offsetmode left, specified as string."""
    genomic_data = request.getfixturevalue(genomic_data_fixture)
    query = Query(
        query_steps=[
            RegionOffsetTransformation("left"),
        ],
    )
    result = query.build(genomic_data).compute()
    # check that the offset is correct
