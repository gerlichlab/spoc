...
@pytest.mark.parametrize("params", product(*CONTACT_PARAMETERS))
def test_write_read_contacts_global_parameters_w_metadata_pandas(
    unlabelled_contacts_2d, params
):
...
@pytest.mark.parametrize("params", product(*CONTACT_PARAMETERS))
def test_write_read_contacts_global_parameters_w_metadata_dask(
    unlabelled_contacts_2d, params
):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        file_name = tmpdirname + "/" + "_".join([str(x) for x in params]) + ".parquet"
        contacts = Contacts(
            dd.from_pandas(unlabelled_contacts_2d, npartitions=2), *params
        )
        FileManager().write_multiway_contacts(file_name, contacts)
        # read contacts
...
@pytest.mark.parametrize("params", product(*CONTACT_PARAMETERS))
def test_write_read_contacts_global_parameters_w_metadata_pandas_to_dask(
    unlabelled_contacts_2d, params
):
...
@pytest.mark.parametrize("params", product(*CONTACT_PARAMETERS))
def test_write_read_contacts_global_parameters_w_metadata_dask_to_pandas(
    unlabelled_contacts_2d, params
):
    """Test writing with pandas and reading with dask"""
    with tempfile.TemporaryDirectory() as tmpdirname:
