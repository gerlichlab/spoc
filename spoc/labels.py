...
class LabelAnnotator():
    ...
    def _assign_sister(self, df_row) -> pd.Series:
    ...
    def annotate_fragments(self, fragments: pd.DataFrame) -> pd.DataFrame:
        ...
        # assign fragments
        fragments_w_label = fragments.assign(is_labelled=self._assign_label_state).dropna(subset=["is_labelled"])
        fragments_w_label.loc[:, "is_labelled"] = fragments_w_label.astype(bool)
        fragments_w_label.loc[:, "sister_identity"] = fragments_w_label.apply(self._assign_sister, axis=1)
