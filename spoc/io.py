...
class FileManager:
    ...
    def load_multiway_contacts(self, path: str, number_fragments: Union[int, None] = None):
        data = pd.read_parquet(path)
        if self._verify_schemas:
            if number_fragments is None:
                raise ValueError("Number of fragments need to be specified is schema is validated!")
